...
void Ghost::setMode(std::shared_ptr<Mode> mode)
{
	_mode = mode;
}

Ghost::Ghost(const char blocks[][gamefield::GAMEFIELD_COLUMN]) :
	Characters(blocks), _targetPoint{ -1, -1 }, _lastPosition{-1, -1}, _stdMode(nullptr), _mode(nullptr), _lastCall(0.0f)
{
}
